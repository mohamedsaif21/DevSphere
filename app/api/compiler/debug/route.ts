...
import { generateGroqText } from '@/lib/groqGenerate';
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const text = await generateGroqText(prompt, apiKey);

    return NextResponse.json({ text });
  } catch (err: unknown) {
    console.error('Error in /api/compiler/debug:', err);
    const message = err instanceof Error ? err.message : 'Failed to get analysis from GROQ';
